...
### Terminal commands
- Before creating redshift cluster ,create the IAM role to be assigned to the cluster    
    ```
    $ python3 create_IAM_Role.py
    ```
- To create the redshift cluster
    ```
...
- Once done with the cluster, you can delete it by running the following:
    ...
    ```
- sql_queries.py : Contains all sql queries for the ETL pipeline
- test.ipynb : to check the data in the cluster.
